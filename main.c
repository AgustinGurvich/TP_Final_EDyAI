 ...
 */
void llenar_camino_final(AlmacenarCamino* camino,
                         AlmacenarCamino* caminoFinal) {
...
/*
 *Recibe dos punteros a una estructura AlmacenarCamino, la cantidad de ciudades
 *del diagrama, la ciudad actual del recorrido la matriz de adyacencia y una
 *matriz de enteros que indica que ciudades fueron visitadas Revisa si
 *encontramos un ciclo posible, caso contrario comienza a elegir las ciudades
 *donde viajar y realizar un recorrido desde allí
 */
void resuelve_tsp(AlmacenarCamino* caminoParcial, AlmacenarCamino* caminoFinal,
                  int cantidadCiudades, int ciudadActual, int** matrizAdyacente,
                  int* visitados) {
  if (caminoParcial->cantidadVisitados == cantidadCiudades - 1 &&
      matrizAdyacente[ciudadActual][0] > 0 &&
      (caminoFinal->costoTotal == -1 ||
       caminoFinal->costoTotal >
           (caminoParcial->costoTotal + matrizAdyacente[ciudadActual][0]))) {
    caminoParcial->listaDeVisitados[caminoParcial->cantidadVisitados][0] =
        ciudadActual;
    caminoParcial->listaDeVisitados[caminoParcial->cantidadVisitados][1] = 0;
    caminoParcial->listaDeVisitados[caminoParcial->cantidadVisitados][2] =
        matrizAdyacente[ciudadActual][0];
    caminoParcial->costoTotal =
        caminoParcial->costoTotal + matrizAdyacente[ciudadActual][0];
    ...
    for (int ciudadDestino = 0; ciudadDestino < cantidadCiudades;
         ciudadDestino++) {
      if ((caminoFinal->costoTotal == -1 ||
           caminoFinal->costoTotal >
               (caminoParcial->costoTotal +
                matrizAdyacente[ciudadActual][ciudadDestino])) &&
          (matrizAdyacente[ciudadActual][ciudadDestino] > 0) &&
          !(visitados[ciudadDestino])) {
        visitados[ciudadDestino] = 1;
        caminoParcial->listaDeVisitados[caminoParcial->cantidadVisitados][0] =
            ciudadActual;
        caminoParcial->listaDeVisitados[caminoParcial->cantidadVisitados][1] =
            ciudadDestino;
        caminoParcial->listaDeVisitados[caminoParcial->cantidadVisitados][2] =
            matrizAdyacente[ciudadActual][ciudadDestino];
        caminoParcial->costoTotal =
            ...
            matrizAdyacente[ciudadActual][ciudadDestino];
        caminoParcial->cantidadVisitados++;
        resuelve_tsp(caminoParcial, caminoFinal, cantidadCiudades,
                     ciudadDestino, matrizAdyacente, visitados);
...
int main(int argc, char* argv[]) {
  if (argc < 3) {
    printf(
        "No se completaron los 3 argumentos necesarios en la ejecucion del "
        "programa");
    return 0;
  }
  ...
  if (cantidadCiudades < 2) {  // Reviso que sea un caso valido
    ...
    int** matrizAdyacente = obtenerCostos(
        file, ciudades, cantidadCiudades);  // Creo la matriz adyacente
    fclose(file);
    ...
    resuelve_tsp(camino, caminofinal, cantidadCiudades, 0, matrizAdyacente,
                 visitados);
    file = fopen(argv[2], "w+");
    for (int i = 0; i < cantidadCiudades; i++) {
      fprintf(file, "%s,%s,%d\n", ciudades[caminofinal->listaDeVisitados[i][0]],
              ciudades[caminofinal->listaDeVisitados[i][1]],
              caminofinal->listaDeVisitados[i][2]);
    }
    fclose(file);
    free(visitados);
